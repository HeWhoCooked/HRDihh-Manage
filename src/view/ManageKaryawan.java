...
public class ManageKaryawan extends javax.swing.JFrame {
    ...
    public ManageKaryawan() {
        initComponents(); // memuat semua komponen dari NetBeans GUI designer
        this.setExtendedState(JFrame.MAXIMIZED_BOTH);

        // 1. Buat model tabel baru dengan kolom yang benar dan override isCellEditable
        DefaultTableModel modelBaru = new DefaultTableModel(
                new String[]{"No", "Nama", "Departemen", "Posisi", "Aksi"}, 0
        ) {
            @Override
            public boolean isCellEditable(int row, int column) {
                return column == 4; // hanya kolom Aksi yang bisa diedit (untuk memicu editor)
            }
        };
        DataTbl.setModel(modelBaru);

        // 2. Atur lebar kolom aksi agar tombol muat
        DataTbl.getColumnModel().getColumn(4).setMinWidth(200);
        DataTbl.getColumnModel().getColumn(4).setPreferredWidth(200);
        DataTbl.getColumnModel().getColumn(4).setMaxWidth(250);

        // 3. Pasang cell renderer & editor (TableActionCell perlu menerima parent frame)
        TableActionCell actionCell = new TableActionCell(DataTbl, this);
        DataTbl.getColumnModel().getColumn(4).setCellRenderer(actionCell);
        DataTbl.getColumnModel().getColumn(4).setCellEditor(actionCell);

        // 4. Styling header tabel (hitam dengan teks putih)
        ...
        // 5. Event untuk tombol Tambah
        TambahKwnBT.addActionListener(e -> {
            new TambahKaryawan().setVisible(true);
            dispose();
        });

        // 6. Muat data karyawan dari database
        muatDataKaryawan();
    }
    ...
    private void initComponents() {
        ...
        TambahKwnBT.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                TambahKwnBTActionPerformed(evt);
            }
        });
    ...
    public void muatDataKaryawan() {
        ...
        for (Pegawai p : karyawanList) {
            model.addRow(new Object[]{
                no++,
                p.getNama(),
                p.getDepartemen().getNama(),
                p.getJabatan().getNama(),
                "" // kolom aksi diisi string kosong (akan diganti tombol oleh cell renderer)
            });
        }
    }
    ...
    private void TambahKwnBTActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_TambahKwnBTActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_TambahKwnBTActionPerformed
